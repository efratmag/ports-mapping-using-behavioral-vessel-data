...
def main(import_path, export_path, col='firstBlip', dbscan_eps=2, dbscan_min_samples=10, outliers_std=3.5, alpha=2):
    ...
    anchoring_df = df[(df['activity'] == 'anchoring') & (df['vessel_class_calc'].isin(['Tanker', 'Cargo']))]

    avg_lat = anchoring_df[col+'_lat'].mean()
    avg_lng = anchoring_df[col + '_lng'].mean()
    ...
    blips_distance_matrix = squareform(pdist(anchoring_df[['firstBlip_lat', 'firstBlip_lng']], (lambda u, v: haversine(u, v))))
    dbscan = DBSCAN(eps=dbscan_eps, min_samples=dbscan_min_samples, metric="precomputed").fit(blips_distance_matrix)

    anchoring_df['labels'] = dbscan.labels_
    anchoring_df['labels'] = 'cluster_' + anchoring_df['labels'].astype(str)

    poly_list = []

    logging.info(f'generating polygons...')
    for label in anchoring_df[(anchoring_df['labels'] != 'cluster_-1')]['labels'].unique():
        cluster_df = anchoring_df[anchoring_df['labels'] == label]
    ...
    anchoring_df.to_csv(os.path.join(export_path, clustering_file_name), index=False)
    polygons_df.to_file(os.path.join(export_path, f'{polyons_file_name}.geojson'), driver='GeoJSON')

    kepler_map.add_data(anchoring_df, 'anchoring_clusters')
